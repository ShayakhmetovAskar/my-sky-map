...
from app.dependencies import get_current_user, get_db, get_hips_storage, get_storage
...
@pytest.fixture
def mock_hips_storage():
    """Public tile bucket (APO-81). No MinIO in unit tests — never a real client."""
    storage = MagicMock(spec=HipsStorage)
    storage.delete_prefix.return_value = 0
    storage.copy_prefix.return_value = 0
    storage.list_prefix.return_value = []
    return storage
...
@pytest.fixture
async def client(mock_storage, mock_hips_storage):
    ...
    app.dependency_overrides[get_hips_storage] = lambda: mock_hips_storage
    ...
    async with test_engine.begin() as conn:
        await conn.execute(text("TRUNCATE tasks, submissions, collections, star_aliases, star_catalog CASCADE"))
