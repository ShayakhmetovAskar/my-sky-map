...
Жизненный цикл папки — `app/services/image_tiles.py` (APO-87):

- **Purge.** `DELETE /submissions/{id}` до commit синхронно чистит `img/{secret}/` каждого
  снимка задачи и приватный `users/{sub}/submissions/{id}/` (вход + output всех задач).
  Ошибка хранилища → **503**, строка в БД не трогается: она — единственный указатель на эти
  объекты, а тайлы читаются без авторизации. Обе половины идемпотентны, повтор дочищает остаток.
- **Ротация.** `rotate_image_secret(task_id)` отзывает ссылку без перенарезки: `copy_prefix` →
  удаление старого префикса → новый `base`/`thumb` в `result.hips`. Старый префикс удаляется
  **до** commit-а намеренно: сорванная ротация должна оставить сломанный снимок (заметно,
  чинится ре-солвом), а не живую отозванную ссылку. Наружу не бросает — вызывается из
  `BackgroundTasks`, где traceback утащил бы секрет в логи; сбой = warning с `redact_secrets`
  и откат частичной копии.
