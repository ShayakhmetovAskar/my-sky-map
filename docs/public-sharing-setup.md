...
- **Ротация секретов снимков при отзыве** — APO-92. Хендлер `unshare_collection` — точка
  подключения, `rotate_image_secret` уже есть (APO-87). До неё уже скачанные и закэшированные
  тайлы остаются читаемыми по старому `base`; сам манифест умирает мгновенно.
